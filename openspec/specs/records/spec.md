...
Record 資料模型與生命週期:`data_json` sparse 儲存(§2.4 格式)、依 field type 的值驗證、CRUD、軟刪除、`source` 來源(manual / screenshot / form),以及支援 filter/sort/limit/offset 的列表 API。所有存取經 `scopedDb(workspace_id)`,確保多租戶隔離。
